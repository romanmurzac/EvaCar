from internal_tools import data_scraper
import var_const
...
for key in range(1, unique_cars + 1):
    for index in range(1, car_identifier[key][1] + 1):
        ...
        try:
            ...
            third_data = data_scraper.get_raw_data(
                endpoint=f"{user_url}/{car_identifier[key][0]}?page={index}",
                html_element=html_elements[3], html_class=html_classes[3])
            data_07 = data_scraper.select_list_elements(raw_data=third_data, index=0, step=2)
            data_7 = data_scraper.split_str_to_elem(raw_data=data_07, splitter=" ", initial_index=0, final_index=1)
            data_8 = data_scraper.split_str_to_elem(raw_data=data_07, splitter=" ", initial_index=1, final_index=2)
            data_9 = data_scraper.select_list_elements(raw_data=third_data, index=1, step=2)

            # Retrieve content from fourth area
            fourth_data = data_scraper.get_raw_data(
                endpoint=f"{user_url}/{car_identifier[key][0]}?page={index}",
                html_element=html_elements[4], html_class=html_classes[4])
            data_10 = fourth_data

            # Retrieve content from fifth area
            fifth_data = data_scraper.get_raw_data(
                endpoint=f"{user_url}/{car_identifier[key][0]}?page={index}",
                html_element=html_elements[3], html_class=html_classes[5])
            data_11 = fifth_data

            # Retrieve content from sixth area
            sixth_data = data_scraper.get_raw_data(
                endpoint=f"{user_url}/{car_identifier[key][0]}?page={index}",
            ...
            data_scraper.save_to_csv(file_name=user_filename, column_names=var_const.variables.raw_schema,
